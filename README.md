...
Jeżeli komunikat nadal pojawia się na wersji 12.4.1:
...
5. sprawdź, czy numer w menu wynosi dokładnie `12.4.1`.
...
- cache pełnego planu sprzed poprawki parsera jest automatycznie odrzucany i pobierany ponownie bez usuwania tokenów, ustawień ani historii;
...
- finalne wymuszenie nowego cache po poprawce ścieżki Render-first;
- test backendu sprawdza najpierw Render/IMAP, jeśli token synchronizacji poczty jest dostępny;
- komunikaty odświeżania odpowiadają rzeczywistemu źródłu danych;
- adres Apps Script może pozostać awaryjny, ale nie blokuje podstawowego odświeżania grafiku.


## Zmiany 12.4.5 — poprawka wyboru aktualnego grafiku

- backend Render 1.4.4 uwzględnia zarówno aktualny adres dyrektora, przekierowania służbowe, jak i historyczny adres `dgorski5@wp.pl` dla starszych wiadomości;
- najnowszy pełny dokument danego tygodnia zastępuje starsze pełne wersje zamiast być z nimi scalany;
...
## Zmiany 12.5.1 — atomowy grafik kanoniczny

- aplikacja przyjmuje tylko kompletną odpowiedź Render, w której `weeks`, `internatWeeks` i `authoritativeWeeks` wskazują tę samą wersję źródła dla każdego tygodnia;
- zduplikowane tygodnie, brak metadanych lub mieszane `sourceVersion` powodują odrzucenie całej odpowiedzi i zachowanie ostatniego poprawnego stanu;
- aplikacja nie łączy danych Render z Apps Script ani ze starym lokalnym indeksem grafików;
- wersja cache 12.5.1 wymusza pobranie poprawionej logiki walidacji.


## Zmiany 12.5.2 — zamrożona historia tygodni

- polityka źródła `latest-document-per-week-v2` identyfikuje wersję tygodnia dokumentem, nie rezultatem parsera;
- ten sam `sourceVersion` nie nadpisuje już zapisanej wersji tygodnia po ponownym odświeżeniu;
- tydzień może zostać zastąpiony wyłącznie przez dokument o nowszym czasie źródłowym/UID;
- brak starego tygodnia w chwilowej odpowiedzi backendu nie usuwa historii lokalnej;
- cache pełnego internatu przechowuje do 60 tygodni;
- zmiana polityki do v2 odrzuca stare niestabilne cache i pobiera jednokrotnie nowy stan kanoniczny.


## Zmiany 12.5.3 — odporne odświeżanie

- migracja polityki grafiku nie kasuje lokalnie zapisanego planu przed udanym pobraniem nowej wersji;
- automatyczne uruchomienie korzysta z szybkiego cache backendu Render;
